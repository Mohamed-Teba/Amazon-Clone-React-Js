...
import { correct } from "../../assets/index"; // Correct icon image
import {
  deleteProduct,
  resetCart,
  increaseQuantity,
  decreaseQuantity,
} from "../../Redux/amazonSlice"; // Redux actions
import { Link, useLoaderData } from "react-router-dom"; // Routing
import { useNavigate } from "react-router-dom";
import { collection, doc, getDoc, setDoc, updateDoc } from "firebase/firestore"; // Firebase Firestore functions
import { db } from "../../firebase.config"; // Firebase config
import { useCart } from "../../context/userCartContext"; // Cart context
import ProductsSlider from "../../Home/productSlider"; // Product slider component

const CartItems = () => {
  const data = useLoaderData(); // Get loader data from route
  const productsData = data.data.products;

  const dispatch = useDispatch();
  const products = useSelector((state) => state.amazon.products); // Redux cart
  ...
  const handleDecreaseQuantity = async (productTitle) => {
    ...
    if (docSnapshot.exists()) {
      ...
      const productIndex = userCartData.findIndex(
        (product) => product.title === productTitle
      );
      if (productIndex !== -1 && userCartData[productIndex].quantity > 1) {
  ...
  // Increase item quantity in Firebase
  ...
  // Delete item from Firebase cart
  const handleDeleteProduct = async (productTitle) => {
    ...
    if (docSnapshot.exists()) {
      const userCartData = docSnapshot.data().cart;
      const updatedCart = userCartData.filter(
        (product) => product.title !== productTitle
      );
  ...
  return (
    <div className="flex flex-col-reverse lgl:flex-row gap-5">
        ...
        <div className="w-full  bg-white py-7 px-5">
          ...
          {userCart.length > 0 ? (
            <div ref={cartRef}>
              {userCart.map((product, index) => (
                <div
                  key={index}
                  className="w-full border-b-[1px] border-b-gray-200 p-4 flex flex-col mdl:flex-row gap-6"
                >
                  <div
                    className="w-full mdl:w-1/5 cursor-pointer"
                    onClick={() =>
                      handleCategoryClick(product.category, product.title)
                    }
                  >
                    <img
                      className="w-[70%] mx-auto h-[70%] mdl:w-48 mdl:h-48"
                  ...
                  <div className="w-4/5 flex flex-col gap-2 -mt-2">
                    <h2
                      className="text-[23px] font-medium cursor-pointer"
                      onClick={() =>
                        handleCategoryClick(product.category, product.title)
                      }
                    >
                    ...
                    <div className="flex flex-row text-sm mdl:text-base gap-5">
                      ...
                      <p className="border-l-[1px] pl-5 border-gray-200 capitalize">
                    ...
                    <div className="flex flex-row justify-between gap-5 mt-2">
                      <div className="flex items-center justify-center">
                        ...
                        <p
                          ...
                          className="px-2 cursor-pointer bg-gray-200 rounded-md hover:bg-gray-400 duration"
                        >
                          -
                        </p>
                  ...
                  <div className="w-4/5 flex flex-col gap-2 -mt-2">
                    ...
                    <div className="flex flex-row justify-between gap-5 mt-2">
                      <div className="flex items-center justify-center">
                        ...
                        <p
                          onClick={() =>
                            dispatch(decreaseQuantity(product.title))
                          }
                          className="px-2 cursor-pointer bg-gray-200 rounded-md hover:bg-gray-400 duration"
                        >
                          -
                        ...
                        <p
                          onClick={() =>
                            dispatch(increaseQuantity(product.title))
                          }
                          className="px-2 cursor-pointer bg-gray-200 rounded-md hover:bg-gray-400 duration"
                        >
                          +
                      ...
                      <button
                        onClick={() => dispatch(deleteProduct(product.title))}
                        className="text-blue-600"
                      >
                        Delete
                      </button>
          ...
          <div className="flex flex-col mdl:flex-row justify-between text-xs mdl:text-base">
            <button
              ...
              className="lgl:w-[200px] border-[1px] bg-gray-100 border-gray-200 py-1 text-sm text-blue-600 rounded-lg text-center p-[4px] mt-1 active:ring-2 active:ring-offset-1 active:ring-blue-600"
            >
              Clear Cart
            </button>
            <div className="text-sm mdl:text-[22px] items-baseline mt-5 mdl:mt-0 font-medium flex justify-center mdl:justify-end">
      ...
      {/* Right Section - Order Summary & Suggestions */}
      <div className="w-[92%] items-center lgl:w-[22%] mx-auto flex flex-col gap-5 my-4 lgl:my-10">
        <div className="w-full bg-white py-6 px-5">
          <div className="flex flex-row gap-2">
            ...
            <span className="text-[13px] text-[#17a34acc]">
          ...
          <div className="text-[18px] mt-4 font-medium flex justify-start items-center">
            SubTotal ({userCart.length > 0 ? cartTotalQty : totalQty} items)
            :&nbsp;
            <div className="flex items-center">
          ...
          <div className="border-[1px] border-gray-200 mt-4 flex items-center justify-center py-2">
            EMI Available
          </div>
        </div>
        ...
        <div className="w-full hidden lgl:block bg-white">
          <h1 className="font-semibold mx-3 pt-3">
            Customers who bought other items
          </h1>
          <div
            style={{ height: productDivHeight }}
            className="bg-white flex flex-col gap-4 py-3 ml-3 custom-scrollbar overflow-y-hidden hover:overflow-y-scroll"
          >
