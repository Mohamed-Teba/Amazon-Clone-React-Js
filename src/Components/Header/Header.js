...
const Header = () => {
  ...
  const product = useLoaderData(); // Load products data from route loader
...
>>>>>>> 065d13bc514f0944cfe658bbdfd72108175af39c
  ...
  // Handle logout and reset all user-related state
  const handleLogout = () => {
    signOut(auth).then(() => {
      dispatch(userSignOut());
      dispatch(setUserAuthentication(false));
        ...
        <div className="w-full bg-amazon_blue text-white px-2 py-2 flex items-center gap-1 xs:justify-evenly">
          ...
          <Link to="/Cart">
            <div className="flex flex-row items-start justify-center headerHover pt-2 relative">
              ...
              <p className="text-xs font-semibold mt-3 text-whiteText">
                Cart
                {/* Show either the value from context or local redux */}
