...
// Import icons for info and exclamation
...
// Import Material-UI icon for arrow
import ArrowRightIcon from "@mui/icons-material/ArrowRight";
// Import loading spinner
import { RotatingLines } from "react-loader-spinner";

// SignUp component - Handles user registration and validation
const SignUp = () => {
  // State for loading spinner and success message
  const [loading, setLoading] = useState(false);
  const [successMsg, setSuccessMsg] = useState("");
  // Firebase authentication instance
  const auth = getAuth();

  // State for error messages and form fields
    ...
    <div className="flex flex-col w-full h-auto">
      ...
      <div className="border border-1 border-gray-300 w-96 rounded-md m-auto h-auto px-4 py-4">
        ...
        <p className="text-xs tracking-wider mt-5">
          ...
          and
          <span className="text-blue-700 hover:text-red-600 hover:underline font-medium cursor-pointer">
            {" "}
            Privacy Policy.
          </span>{" "}
        </p>
