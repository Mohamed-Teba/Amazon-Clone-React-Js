  ...
  return (
    <div className="mb-10 flex flex-row flex-wrap justify-center gap-5 relative">
      {categoryData.map((item) => (
        <div
          key={item.id}
          className="w-[355px] bg-white border-[1px] border-gray-200 py-6 hover:border-transparent shadow-none hover:shadow-testShadow duration-200 flex flex-col gap-4"
        >
          <div className="text-2xl ml-2 font-semibold tracking-wide">
            {item.plot}
          </div>
          {item.subcategories ? (
            <div className="w-64 mx-auto flex  flex-row flex-wrap gap-3  justify-between">
              {item.subcategories.map((item, index) => (
                ...
                >
                  ...
                  <h4 className="text-xs font-semibold mx-auto mt-1 mb-2 group-hover:text-red-400">
                    {item.title}
                  </h4>
                </div>
              ))}
                    ...
                    <div key={item.id} className='w-[355px] bg-white border-[1px] border-gray-200 py-6 hover:border-transparent shadow-none hover:shadow-testShadow duration-200 flex flex-col gap-4' >
                        <div className='text-2xl ml-2 font-semibold tracking-wide'>{item.plot}</div>
                        {
                            item.subcategories ?
                                <div className='w-64 mx-auto flex  flex-row flex-wrap gap-3  justify-between'>
                                    {item.subcategories.map((item, index) => (
                                        <div key={index} className='group flex flex-col items-center justify-center gap-1 mb-2 cursor-pointer' onClick={() => handleCategoryClick(item.category)} >
                                            <img className='w-28 h-28' src={item.img} alt='img' />
                                            <h4 className='text-xs font-semibold  group-hover:text-red-400'>{item.title}</h4>
                                        </div>
                                    ))}
                                </div>
                                :
                                    <div className='group flex flex-col items-center justify-center gap-1 mb-2 mx-5 cursor-pointer' onClick={() => handleCategoryClick(item.category)}>
...
<<<<<<< HEAD
              ...
              <p className="text-cyan-700 text-sm font-medium ml-[20px] group-hover:text-red-400">
                See more
              </p>
            </div>
          )}
